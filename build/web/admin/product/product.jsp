  ...
  <div class="row">
    ...
    <main class="col-md-10 ms-sm-auto col-lg-10 px-md-4 py-4">
      ...
      <form action="${pageContext.request.contextPath}/admin/manage-product" method="GET" class="row g-3 mb-4">
        ...
        <div class="col-md-3">
          <input type="text" name="search" class="form-control" placeholder="Tìm theo mã, tên, NCC..."
        ...
        <div class="col-md-3">
          <select name="supplierId" class="form-select">
            ...
            <c:forEach var="supplier" items="${suppliers}">
              <option value="${supplier.supplierId}" <c:if test="${supplierId == supplier.supplierId}">selected</c:if>>
        ...
        <div class="col-md-2">
          <input type="number" name="minPurchasePrice" class="form-control" placeholder="Giá mua >=" value="${minPurchasePrice}"/>
        </div>
        <div class="col-md-2">
          <input type="number" name="minSalePrice" class="form-control" placeholder="Giá bán >=" value="${minSalePrice}"/>
        </div>
        <div class="col-md-2">
            ...
            <c:when test="${not empty products}">
              <c:forEach var="product" items="${products}" varStatus="loop">
                <tr>
                  ...
                  <td>
                    <c:choose>
                      <c:when test="${not empty product.supplierId}">
                  ...
                  <td>
                    <c:choose>
                      ...
                      <c:otherwise><span class="badge bg-secondary">Không HĐ</span></c:otherwise>
                    </c:choose>
                  </td>
                  <td>
                    ...
                    <button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete('${product.productId}', '${fn:escapeXml(product.productName)}')">Xóa</button>
                  </td>
                </tr>
              </c:forEach>
            </c:when>
  ...
  document.addEventListener("DOMContentLoaded", () => {
    document.getElementById("confirmDeleteBtn").addEventListener("click", () => {
      if (productIdToDelete) {
        window.location.href = `${"${pageContext.request.contextPath}"}/admin/manage-product?action=delete&id=` + productIdToDelete;
      }
      bootstrap.Modal.getInstance(document.getElementById("deleteConfirmModal")).hide();
    });

    // Hiển thị toast nếu có message
    const toastMessage = "${fn:escapeXml(sessionScope.toastMessage)}";
    const toastType = "${fn:escapeXml(sessionScope.toastType)}";
