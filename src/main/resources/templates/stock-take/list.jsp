        ...
        <main class="col-md-10 ms-sm-auto col-lg-10 px-md-4 py-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <c:if test="${currentUser.roleId == 'warehouse_staff'}">
                    <a href="${pageContext.request.contextPath}/stock-take?action=create" class="btn btn-success">+ Tạo
                        Phiếu Kiểm Kê</a>
                </c:if>
            </div>
                    ...
                    <form method="GET" action="${pageContext.request.contextPath}/stock-take">
                        <div class="input-group">
                            <select name="status" class="form-select">
                                ...
                                <option value="in_progress" ${param.status == 'in_progress' ? 'selected' : ''}>Đang kiểm
                                    kê
                                </option>
                                <option value="completed" ${param.status == 'completed' ? 'selected' : ''}>Hoàn thành
                                </option>

                                <option value="reconciled" ${param.status == 'reconciled' ? 'selected' : ''}>Đã đối
                                    soát
                                </option>
                            </select>
                        ...
                        <tr>
                            ...
                            <td>
                                <c:if test="${stockTake.totalProducts != null && stockTake.totalProducts > 0}">
                                    <c:set var="progress"
                                           value="${(stockTake.completedProducts * 100) / stockTake.totalProducts}"/>
